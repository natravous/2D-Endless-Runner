...
public class TerrainGeneratorController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        {
            GeneratedTerrain(lastGeneratedPositionX, terrain);
            lastGeneratedPositionX += terrainTemplateWidth;
        }

        while(lastGeneratedPositionX < GetHorizontalPositionEnd())
        {
            GeneratedTerrain(lastGeneratedPositionX);
            lastGeneratedPositionX += terrainTemplateWidth;
        }
    }
    ...
    private void GeneratedTerrain(float posX, TerrainTemplateController forceterrain = null)
    {
        GameObject newTerrain = Instantiate(terrainTemplates[Random.Range(0, terrainTemplates.Count)].gameObject, transform);

        newTerrain.transform.position = new Vector2(posX, positionTerrainY); //position
